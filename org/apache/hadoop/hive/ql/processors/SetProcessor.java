...
public class SetProcessor implements CommandProcessor {
  ...
  private void dumpOption(String s) {
    ...
    if (ss.getConf().get(s) != null) {//如果有，就把值打印出来
  ...
  private CommandProcessorResponse getVariable(String varname){
    ...
    if (varname.startsWith(SetProcessor.SYSTEM_PREFIX)){// system:
      String propName = varname.substring(SetProcessor.SYSTEM_PREFIX.length());
    ...
    } else if (varname.indexOf(SetProcessor.ENV_PREFIX)==0){// env:
      ...
      dumpOption(varname);
      return new CommandProcessorResponse(0, null, null, getSchema());// 0是表示成功了！！
    }
  }
...
// create a Schema object containing the give column
  private Schema getSchema() {
    ...
    tmpFieldSchema.setName(SET_COLUMN_NAME);//set

    tmpFieldSchema.setType(STRING_TYPE_NAME);//"string"
    sch.putToProperties(SERIALIZATION_NULL_FORMAT, defaultNullString);//"serialization.null.format","\\N"
