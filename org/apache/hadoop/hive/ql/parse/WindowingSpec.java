...
public class WindowingSpec {
  ...
  public void addWindowFunction(WindowFunctionSpec wFn) {
    ...
    aliasToWdwExpr.put(wFn.getAlias(), wFn); // _wcol0
  }
  ...
  public void validateAndMakeEffective() throws SemanticException {
    for(WindowExpressionSpec expr : getWindowExpressions()) {
      ...
      // 一个没有Parition Spec的Window Spec，设置一个常数0作为Partitioned标志
      ...
      throws SemanticException
  {
    if (sourceId != null) // w1， 这个判断，是为了在后面递归调用的时候，直接跳出返回
    {
      ...
      {
        dest.setPartition(source.getPartition()); // partitionColumns=[(TOK_TABLE_OR_COL p_mfgr)]
      }

      if (dest.getOrder() == null)
      {
        dest.setOrder(source.getOrder()); // orderColumns=[(TOK_TABLE_OR_COL p_name) ASC]
      }

      if (dest.getWindowFrame() == null)
      {
        dest.setWindowFrame(source.getWindowFrame()); // end 和 start
      }

      visited.add(sourceId);
      // 正常情况下，source里面的sourceid是null
  ...
  private void effectiveWindowFrame(WindowFunctionSpec wFn, WindowSpec wdwSpec) {
    ...
    if ( wFrame == null ) {
      if (!supportsWindowing ) { // 如果不支持windowing，只需要处理last_value函数
  ...
  private void validateWindowFrame(WindowSpec wdwSpec) throws SemanticException {
    ...
    // 方向是following，但是unbounded是不可以的（例如，一个合理范围可以是FOLLOWING 2 to FOLLOWING 5）
  ...
  private void validateValueBoundary(BoundarySpec bs, OrderSpec order) throws SemanticException {
    if ( bs instanceof ValueBoundarySpec ) {
      ...
      vbs.setExpression(order.getExpressions().get(0).getExpression()); // 设置排序的key
    }
  }
