...
public class Driver implements CommandProcessor {
  ...
  private CommandProcessorResponse runInternal(String command) throws CommandNeedRetryException {
    ...
    int ret;
    synchronized (compileMonitor) {Long start = System.currentTimeMillis(); LOG.info("start-compile:" + command + "|" + sFormat.format(new Date(start)));
      ret = compile(command);LOG.info("end-compile:" + command + "|consume-time:" + (System.currentTimeMillis() - start));
    }
    if (ret != 0) {
