...
public class DDLTask extends Task<DDLWork> implements Serializable {
  ...
  public void initialize(HiveConf conf, QueryPlan queryPlan, DriverContext ctx) {
    ...
    formatter = MetaDataFormatUtils.getFormatter(conf);// TextMetaDataFormatter
    INTERMEDIATE_ARCHIVED_DIR_SUFFIX =
      HiveConf.getVar(conf, ConfVars.METASTORE_INT_ARCHIVED);// _INTERMEDIATE_ARCHIVED
    INTERMEDIATE_ORIGINAL_DIR_SUFFIX =
      HiveConf.getVar(conf, ConfVars.METASTORE_INT_ORIGINAL);// _INTERMEDIATE_ORIGINAL
    INTERMEDIATE_EXTRACTED_DIR_SUFFIX =
      HiveConf.getVar(conf, ConfVars.METASTORE_INT_EXTRACTED);// _INTERMEDIATE_EXTRACTED
  }
  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      db = Hive.get(conf);
      //注意work里面的 input  output 和某个不是null的 ***Desc
  ...
  */
  ...
  private int showDatabases(Hive db, ShowDatabasesDesc showDatabasesDesc) throws HiveException {
    ...
    DataOutputStream outStream = null;
    try {
      Path resFile = new Path(showDatabasesDesc.getResFile());//file:/tmp/root/hive_2014-07-15_02-19-41_502_1329606294782422184-1/-local-10000
      FileSystem fs = resFile.getFileSystem(conf);
      outStream = fs.create(resFile);
  ...
  private int createTable(Hive db, CreateTableDesc crtTbl) throws HiveException {
    // create the table
    Table tbl = db.newTable(crtTbl.getTableName());//设置了当前db
    // 设置各种cols
    ...
    if (crtTbl.getCols() != null) {// [FieldSchema(name:member_id, type:string, comment:null), FieldSchema(name:name, type:string, comment:null)]
    ...
    tbl.setInputFormatClass(crtTbl.getInputFormat());// TextInputFormat
    tbl.setOutputFormatClass(crtTbl.getOutputFormat());// HiveIgnoreKeyTextOutputFormat

    tbl.getTTable().getSd().setInputFormat(// TextInputFormat
      tbl.getInputFormatClass().getName());
    tbl.getTTable().getSd().setOutputFormat(
      tbl.getOutputFormatClass().getName());// HiveIgnoreKeyTextOutputFormat
