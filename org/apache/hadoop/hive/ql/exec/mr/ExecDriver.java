...
public class ExecDriver extends Task<MapredWork> implements Serializable, HadoopJobExecHook {
  ...
  public int execute(DriverContext driverContext) {
    ...
    ReduceWork rWork = work.getReduceWork();//如果序列化之前是一个MapWork，那这里会得到null
    ...
    try {
      ...
      emptyScratchDirStr = ctx.getMRTmpFileURI();// /tmp/root/hive_2014-08-01_00-51-37_292_828377430055986989-1/-mr-10000
      emptyScratchDir = new Path(emptyScratchDirStr);
      FileSystem fs = emptyScratchDir.getFileSystem(job);
      fs.mkdirs(emptyScratchDir); // mapreduce的执行目录
    } catch (IOException e) {
      e.printStackTrace();
    ...
    try {
      job.setPartitionerClass((Class<? extends Partitioner>) (Class.forName(HiveConf.getVar(job,
          HiveConf.ConfVars.HIVEPARTITIONER))));// io.DefaultHivePartitioner
    ...
    try{
      ...
      Utilities.createTmpDirs(job, rWork);// 没有就直接退出了
      // 在这里就直接提交到hadoop上面了！！！
  ...
  public static void main(String[] args) throws IOException, HiveException {
    // [-plan, file:/tmp/root/hive_2014-09-04_00-37-23_595_1803755893936223755-1/-local-10003/plan.xml, -jobconffile, file:/tmp/root/hive_2014-09-04_00-37-23_595_1803755893936223755-1/-local-10002/jobconf.xml]
    ...
    try {
      for (int i = 0; i < args.length; i++) {
        if (args[i].equals("-plan")) {
          planFileName = args[++i]; // 把 -plan 后面那个字符串赋给planFileName 下面类推
    ...
    Path p = new Path(planFileName);// file:/tmp/hive-root/hive_2014-08-01_00-25-06_452_6744654297129242222-1/-local-10004/plan.xml
    ...
    if (localtask) {
      ...
      ret = ed.execute(new DriverContext());//ExecDriver.execute
    }
