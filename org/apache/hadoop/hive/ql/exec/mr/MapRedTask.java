...
public class MapRedTask extends ExecDriver implements Serializable {
  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      ...
      // enable assertion
      String hadoopExec = conf.getVar(HiveConf.ConfVars.HADOOPBIN);System.out.println("---hadoopExec:" + hadoopExec);
      String hiveJar = conf.getJar();
      hiveJar = "/root/Desktop/aaa-baishuo.jar";
      ...
      String hiveConfArgs = generateCmdLine(conf, ctx);// 这里会产生-local 和jobconf.xml

      // write out the plan to a local file
      Path planPath = new Path(ctx.getLocalTmpFileURI(), "plan.xml");
      OutputStream out = FileSystem.getLocal(conf).create(planPath);// -local-10004/plan.xml
      ...
      String remoteDebug = ""; // " -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=28000 ";
      hadoopExec = hadoopExec + remoteDebug;
      String cmdLine = hadoopExec + " jar " + jarCmd + " -plan "
          + planPath.toString() + " " + isSilent + " " + hiveConfArgs;
      // /root/workspace/disk2/opensource/hive-release-0.12.0/testutils/hadoop jar /root/workspace/disk2/opensource/hive-release-0.12.0/build/ql/hive-exec-0.12.0.jar org.apache.hadoop.hive.ql.exec.mr.ExecDriver  -plan file:/tmp/hive-root/hive_2014-07-31_02-21-45_332_2912688879867288853-1/-local-10004/plan.xml   -jobconffile file:/tmp/hive-root/hive_2014-07-31_02-21-45_332_2912688879867288853-1/-local-10003/jobconf.xml
      ...
      LOG.info("Executing: " + cmdLine);System.out.println("---Executing: " + cmdLine);
      ...
      // Run ExecDriver in another JVM  就是这句写的文件！！！ 但是不会调试！！！
