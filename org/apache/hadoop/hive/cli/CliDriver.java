...
public class CliDriver {
  ...
  public int processCmd(String cmd) {
    ...
    String[] tokens = tokenizeCmd(cmd_trimmed);//仅仅split了一下
    int ret = 0;
  ...
  int processLocalCmd(String cmd, CommandProcessor proc, CliSessionState ss) {
    ...
    do {
      try {
        needRetry = false;
        if (proc != null) {
          if (proc instanceof Driver) {//如果是set的话就不是driver,这是由CommandProcessorFactory创建的
            ...
            String cmd_1 = getFirstCmd(cmd.trim(), firstToken.length());//得到第一个命令

            if (ss.getIsVerbose()) {
              ss.out.println(firstToken + " " + cmd_1);
            }
            CommandProcessorResponse res = proc.run(cmd_1);//在这里执行
  ...
  public int processLine(String line, boolean allowInterupting) {
    ...
    try {
      ...
      String command = "";
      for (String oneCmd : line.split(";")) {// 按分号分割成单个的hive命令
        ...
        ret = processCmd(command);//处理command
        //wipe cli query state
        SessionState ss = SessionState.get();//如果数据库没起，会抛出异常
  ...
  public void processInitFiles(CliSessionState ss) throws IOException {
    ...
    ss.setIsSilent(saveSilent);//false
  }
  ...
  private  int executeDriver(CliSessionState ss, HiveConf conf, OptionsProcessor oproc)
    ...
    cli.processSelectDatabase(ss);//默认没做什么
    ...
    try {
      if ((new File(historyDirectory)).exists()) {
        String historyFile = historyDirectory + File.separator + HISTORYFILE;///root/.hivehistory
        reader.setHistory(new History(new File(historyFile)));//设置his 文件
      } else {
    ...
    while ((line = reader.readLine(curPrompt + "> ")) != null) {
      ...
      if (line.trim().endsWith(";") && !line.trim().endsWith("\\;")) {
        line = prefix + line;
        ret = cli.processLine(line, true);//跟进去
        prefix = "";
